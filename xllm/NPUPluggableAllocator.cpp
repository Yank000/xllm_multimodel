...
void NPUPluggableAllocator::emptyCacheImpl(bool check_error,
                                           bool free_physical) {
  if (reset_fn_) {
    reset_fn_(check_error);
  }
}
...
void NPUPluggableAllocator::eraseStreamWithBlockPtr(void* block_ptr,
                                                    c10_npu::NPUStream stream,
                                                    void* work_ptr) {
  if (erase_stream_fn_) {
    erase_stream_fn_(block_ptr, stream);
  }
}
...
void NPUPluggableAllocator::recordHcclWorkForBlock(void* block_ptr,
                                                   void* work_ptr) {
  TORCH_NPU_WARN(
...
void NPUPluggableAllocator::copy_data(void* dest,
                                      const void* src,
...
c10_npu::NPUCachingAllocator::ShareableHandle
NPUPluggableAllocator::shareIpcHandle(void* ptr) {
  TORCH_NPU_WARN(
