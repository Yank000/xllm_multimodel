...
void DistManager::setup_multi_node_workers(
  ...
  for (size_t i = 0; i < local_device_indices.size(); ++i) {
    const int32_t local_device_idx = local_device_indices[i];
    const int32_t actual_rank =
        options.node_rank() * each_node_ranks + local_device_idx;
    ...
    servers_.emplace_back(
        std::make_unique<WorkerServer>(local_device_idx,
                                       master_node_addr,
                                       // done,
                                       local_dones[i],
                                       parallel_args,
