...
void ContinuousScheduler::handle_prefill_requests(
  ...
  while (!waiting_priority_queue.empty() && remaining_seq_budget > 0 &&
    ...
    waiting_priority_queue.pop();
    // measure resume stall: time between preemption and successful re-schedule.
    if (request->preempted()) {
      auto it = preempt_start_time_.find(request->request_id());
      if (it != preempt_start_time_.end()) {
        int64_t stall_ms = absl::ToInt64Milliseconds(absl::Now() - it->second);
        LOG(WARNING) << "[SCHED_RESUME] request_id=" << request->request_id()
                     << " stall_ms=" << stall_ms;
        preempt_start_time_.erase(it);
      }
    }
  ...
  while (!running_queue->empty() &&
    ...
    if (options_.enable_online_preempt_offline() && !request->offline() &&
      ...
      if (request_to_preempt.get() != request.get()) {
        ...
        // recompute cost = tokens already generated by the victim that will be
        // thrown away and recomputed on resume (prefix cache disabled).
        size_t recompute_tokens = 0;
        for (const auto& seq : request_to_preempt->sequences()) {
          recompute_tokens += seq->num_generated_tokens();
        }
        // record preempt time to measure resume stall later.
        preempt_start_time_[request_to_preempt->request_id()] = absl::Now();
